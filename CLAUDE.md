...
Next: fab the **V5 spin** — layout complete: J14 mirror routed (the re-route came out *cleaner*, eliminating a back-side trace), H1–H4 adopted in place, J9's +5V feed restored (it had been orphaned by the J14 re-route — caught by headless DRC's unconnected-items check), DRC 0/0, gerbers exported and committed (`pcb/gerbers/ti99-kb-adapter_r5.zip`, ready for JLCPCB: 2-layer, 1.6mm, HASL). On arrival: loopback-test modules (shunt fixture down J10 + J15 wire), build, flash the v5-branch firmware, verify with a straight ribbon. Merge the branch → `main` once bench-verified, then 3D-printed enclosure/mount and a larger order.
