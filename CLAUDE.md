...
- **Rev 4 fix (implemented on the `v4` branch):** firmware mirrors every matrix line (each TI function drives/reads the GPIO whose J10 pin is 16−*p*), and the board gives **every J10 pin — including alpha lock — a BOB channel and a GPIO**, so any future re-pinning is firmware-only, never a new layout. J10 pin 6 carries INT9 and pin 10 carries ALPHA_LOCK (GPIO9, parked as INPUT — software alpha lock unchanged); through the flat ribbon they land on TI-mb pins 10 and 6. Channel fabric (per Jon's map): BOB#1 = J10 1–4 ← GPIO4–7; BOB#2 = J10 5–8 ← GPIO15–18; BOB#3 = J10 9–12 ← GPIO8–11 (GPIO8 newly in service); BOB#4 = J10 13–15 ← GPIO12–14 (CH1 spare). Schematic + firmware verified consistent (60-point netlist check) and compiling. **v4 firmware must not be flashed onto a rev-3 board with a twisted ribbon** (double reversal scrambles the matrix) — the rev-3 baseline is tagged `v3`. J20 gets identical nets to J10; an original keyboard there is expected to mate mirrored like the mb header (bench-unverified).
