...
Next: **V5 spin** — the schematic already carries the J14 mirror and schematic-owned mounting holes; the layout needs a short pass (re-route J14's four traces, adopt H1–H4 per the migration note above), then fresh gerbers and fab. On arrival: loopback-test modules (shunt fixture down J10 + J15 wire), build, flash the v4-branch firmware, verify with a straight ribbon. Merge the branch → `main` once bench-verified, then 3D-printed enclosure/mount and a larger order. (The committed `ti99-kb-adapter_r4.zip` gerbers predate the J14 mirror — a board fabbed from them needs the flipped-MB-plug workaround.)
