...
**V7 (in progress on the `v7` branch): discretize the level shifters + JLC assembly.** The 4 BOB-12009 modules + sockets are replaced by 16 on-board channel cells — per channel one **BSS138** (Q1–Q16, SOT-23, LCSC **C78284**) with source on the LV net / drain on the HV net / gate on 3V3, plus two **10k 0805** pull-ups (R1–R32, LCSC **C17414**, JLC Basic). Same proven topology, zero firmware change, ~$15/board cheaper, and **JLCPCB populates all 48 SMT parts** (kills the hand-soldered-BOB-joint failure mode; connectors remain hand-soldered). Status: schematic generated + verified (netlist, LCSC fields, and a zero-tolerance collinear-wire-overlap check — overlapping stubs silently merge nets, caught twice during development); `pcb/export_jlc.py` produces the JLC assembly BOM (works now) and CPL (after layout). Remaining: F8 + place/route the 48 SMT parts in KiCad (board can shrink), then gerbers + BOM + CPL for the assembly order. **Always eyeball JLC's part-placement preview** — their rotation convention for SOT-23 can differ. V7's shifter section carries directly into the **B-P board** (`MX_KEYBOARD_CONCEPT.md`).
