...
- **Level shifters:** BSS138 discrete MOSFET shifters, 10kΩ pull-ups to 3V3 (LV) and 5V (HV). **V7+: 16 on-board channel cells** (BSS138 SOT-23 LCSC `C78284` + 2× 10k 0805 LCSC `C17414` each, JLC-assembled). Rev ≤6 used 4× BOB-12009 daughterboards (SparkFun topology) in sockets — see the footprint reference below, kept for servicing those boards.
...
- **Rev 3 (BSS138 — superseded):** corrects the rev-2 power/GND swap and carries the **second parallel 15-pin keyboard connector (J20)**, so an original TI keyboard can run alongside the modern USB/BLE keyboard (shared matrix, either can drive a press). Matrix, typing, and SHIFT/CTRL/FCTN modifiers all work. **Known issue:** the keyboard ribbon must be **twisted 180° (a pin reversal)** to work — see below. Gerbers committed to repo.
- **Rev 5 / V5 (BSS138 — fabbed, bench-verified):** the straight-ribbon rework — firmware mirror + full 16-channel fabric (alpha lock on GPIO3, SPARE repair channel → J15), J13 1.1mm cable holes, J14 mirror-wired power output, M2 mounting holes (schematic-owned). Fabbed via JLCPCB (`ti99-kb-adapter_r5.zip`), assembled and **verified working on the console August 2026** — typing, straight ribbon, daisy-chain power all good. One shortcoming: bottom-edge power-connector placement made console power routing awkward. (No rev 4 was ever fabbed — that design was superseded by V5 before ordering.)
- **Rev 7 / V7 (ordered from JLCPCB 2026-08-07 — $56.88 total for 10 boards WITH SMT assembly and 2–4 day fast shipping; ≈$5.70/board all-in):** discrete level shifters replace the BOB daughterboards — 16 on-board BSS138 cells (C78284) + 32× 10k 0805 (C17414), JLC-assembled, all top-side. Layout reported cleaner than the BOB topology. Verdict of the discrete-vs-BOB experiment: ~$17/board of modules+sockets+hand-soldering collapses to ~$2.89 all-in with machine-soldered joints. Connectors + ESP32 module remain hand-soldered. **Boards arrived and verified 2026-09-01: the first assembled unit passed the full loopback (all 16 channels through the machine-soldered fabric) on the first attempt, then ran the real firmware on the console — typing works.** V7 is fully bench-verified; `v7` merged to `main` and tagged.
- **Rev 6 / V6 (ordered from JLCPCB 2026-08-07):** V5 electrically unchanged; power connectors J9/J13/J14 moved to the **top edge** for better console power routing, power runs upsized to 0.75–1.0mm. Gerbers `ti99-kb-adapter_r6.zip`, DRC 0/0, netlist re-verified.
...
- **V5 fix (implemented on the `v5` branch — named `v4` before the spin was renumbered):** firmware mirrors every matrix line (each TI function drives/reads the GPIO whose J10 pin is 16−*p*), and the board gives **every J10 pin — including alpha lock — a BOB channel and a GPIO**, so any future re-pinning is firmware-only, never a new layout. J10 pin 6 carries INT9 and pin 10 carries ALPHA_LOCK (GPIO3, parked as INPUT — software alpha lock unchanged; the strapping quirk is harmless on a never-driven line); through the flat ribbon they land on TI-mb pins 10 and 6. Channel fabric (per Jon's map): BOB#1 = J10 1–4 ← GPIO4–7; BOB#2 = J10 5–8 ← GPIO15–18; BOB#3 = J10 9–12 ← GPIO8, 3, 9, 10; BOB#4 = J10 13–15 ← GPIO11–13. The 16th channel is a **SPARE repair path**: GPIO14 → BOB#4-CH1 → J15 solder pad — any dead channel is recoverable with one jumper + a firmware remap. Mounting holes changed to **2.2mm (M2)** — sized for self-tapping into the planned printed mount (rev 1–3 were 3mm/M3). Superseded before fab by the **V5 spin** (J14 mirror + schematic-owned holes); the interim r4 gerbers were removed from the repo (git history has them at `f2d0bfa`). Schematic + firmware verified consistent (60-point netlist check) and compiling. **v5 firmware must not be flashed onto a rev-3 board with a twisted ribbon** (double reversal scrambles the matrix) — the rev-3 baseline is tagged `v3`. J20 gets identical nets to J10. **Original keyboard on V5's J20: plug it in rotated 180° from its OEM/rev-3 orientation** — bench fact: the keyboard mated *straight* onto rev-3's J20 (and onto the TI mb), and V5 moved the mirror compensation from the twisted cable into the nets, so the plug flips to match. Getting it backwards is harmless (all lines are 10k-pulled open-drain signals) and instantly obvious (scrambled typing → flip).

**2. J13 daisy-chain solder holes were oversized — fixed in source.** The cable-solder holes were 2.0mm drill / 3.0mm pad (originally cut for 14 AWG), so the thin 18-22 AWG power wire floated with no solid joint. Reduced to **1.1mm drill / 2.2mm pad** in `generate_parts.py` and regenerated `J13-CableHeader-1x04.kicad_mod`. The change is in the source now; it lands physically at the V5 fab.

**3. J14 daisy-chain output mates the TI-MB plug backwards — fixed in source, lands at the V5 spin.** Passthrough-mirror principle (same class as the keyboard ribbon): in the original PSU↔MB joint the two connector halves *face each other*; a passthrough board mounts both its interfaces facing the *same* direction, so exactly one must be mirror-wired. J13 is hand-soldered (self-correcting); J14 mates the keyed MB plug, so J14 now carries the mirror: **1=+5V, 2=GND, 3=+12V, 4=−5V** (J13 stays 1=−5V, 2=+12V, 3=GND, 4=+5V). Boards wired straight (rev 1–4): mate the MB plug **flipped 180°** (trim the friction-lock wall if needed) and **meter all four rails before first power** — plugged per the key, the MB would get +12V on GND.

**4. Mounting holes are now schematic-owned** (H1–H4, `MountingHole_2.2mm_M2`) so Update-PCB-from-Schematic can never silently delete them again. Migrating an existing layout: rename the board-only hole footprints to H1–H4, untick "Not in schematic" on each, run F8 with "re-link footprints to symbols by reference" — they're adopted in place.
...
**V7 (in progress on the `v7` branch): discretize the level shifters + JLC assembly.** The 4 BOB-12009 modules + sockets are replaced by 16 on-board channel cells — per channel one **BSS138** (Q1–Q16, SOT-23, LCSC **C78284**) with source on the LV net / drain on the HV net / gate on 3V3, plus two **10k 0805** pull-ups (R1–R32, LCSC **C17414**, JLC Basic). Same proven topology, zero firmware change, ~$15/board cheaper, and **JLCPCB populates all 48 SMT parts** (kills the hand-soldered-BOB-joint failure mode; connectors remain hand-soldered). Status: **layout complete and fab-ready** — schematic and board verified end-to-end (netlist checks incl. a zero-tolerance collinear-wire-overlap check, plus a pad-net sweep of all 16 cells on the routed board; DRC 0/0). Jon reports the discrete footprints lay out *cleaner* than the BOBs. The order package is committed in `pcb/gerbers/`: **`ti99-kb-adapter_r7.zip` + `ti99-kb-adapter_BOM.csv` + `ti99-kb-adapter_CPL.csv`** (48 placements, all top-side). Development note: the new symbol code initially attached FET wires at vertically mirrored pin positions (KiCad symbol +Y is up, sheet +Y is down — see `add_simple`), swapping gate/drain nets on every FET; caught by the board-level pad-net check, cost one re-route session. **Always eyeball JLC's part-placement preview** — their rotation convention for SOT-23 can differ. V7's shifter section carries directly into the **B-P board** (`MX_KEYBOARD_CONCEPT.md`).
